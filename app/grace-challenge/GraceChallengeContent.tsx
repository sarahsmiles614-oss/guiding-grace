...
export default function GraceChallengeContent() {
  ...
  async function handleHeart(postId: string, postUserId: string) {
    if (!userId || postUserId === userId) return;
            ...
            <>
              ...
              <div className="space-y-4">
                <p className="text-xs text-gray-400 uppercase tracking-widest">
                  {revealed ? "Community Responses — Hearts Revealed" : "Community Responses — Hearts hidden until 7am"}
                </p>
                {sortedPosts.map(post => (
                  <div key={post.id} className={`bg-white rounded-2xl shadow-sm border p-5 ${mostLoved?.id === post.id && revealed ? "border-yellow-300" : "border-purple-100"}`}>
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <button
                        onClick={() => handleHeart(post.id, post.user_id)}
