...
export default function TestimonyWallPage() {
  ...
  async function handleSubmit() {
    ...
    setSubmitting(true);
    await supabase.from("testimonies").insert({ user_id: userId, display_name: userName, story });
    setStory("");
  ...
  async function handleHeart(id: string) {
    ...
    if (userHearts.includes(id)) {
      ...
      await supabase.from("testimonies").update({ heart_count: (testimonies.find(t => t.id === id)?.heart_count || 1) - 1 }).eq("id", id);
      setUserHearts(h => h.filter(x => x !== id));
    } else {
      await supabase.from("testimony_hearts").insert({ user_id: userId, testimony_id: id });
      await supabase.from("testimonies").update({ heart_count: (testimonies.find(t => t.id === id)?.heart_count || 0) + 1 }).eq("id", id);
          ...
          <div className="space-y-4">
            {testimonies.map(t => (
              <div key={t.id} className="bg-white rounded-2xl shadow-sm border border-purple-100 p-6">
                <p className="text-xs text-purple-400 mb-2">{t.display_name}</p>
                ...
                <button onClick={() => handleHeart(t.id)} className="flex items-center gap-1 text-sm transition">
                  ...
                  <span className="text-gray-400 text-xs">{t.heart_count || 0}</span>
                </button>
              </div>
            ))}
          </div>
